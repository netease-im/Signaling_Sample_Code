...
#ifndef NIM_SDK_CPP_MSGLOG_HELPER_H_
#define NIM_SDK_CPP_MSGLOG_HELPER_H_
...
struct NIM_SDK_CPPWRAPPER_DLL_API MessageStatusChangedResult
{
	...
	{
        rescode_ = static_cast<NIMResCode>(rescode);
		Json::Value values;
		Json::Reader reader;
		if (reader.parse(result, values) && values.isArray())
		{
            ...
            for (size_t i = 0; i < size; i++)
			{
                ...
                changed.status_ = static_cast<NIMMsgLogStatus>(values[i][kNIMMsglogStatusChangedKeyStatus].asUInt());
...
#endif //NIM_SDK_CPP_MSGLOG_HELPER_H_
